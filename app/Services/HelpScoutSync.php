...
class HelpScoutSync
{
    ...
    private function hsAccessToken(): string
    {
        return Cache::remember('hs_access_token', now()->addMinutes(30), function () {
            ...
            if ($refresh === '') {
                throw new \RuntimeException('No Help Scout refresh token (HS_REFRESH_TOKEN).');
            }

            $resp = Http::asForm()
                ->timeout(8)
                ->post($this->hsTokenUrl, [
            ...
            if (!$resp->ok()) {
                throw new \RuntimeException('HS refresh failed: '.$resp->status().' '.$resp->body());
            }
            $data = $resp->json();
    ...
    private function hsFindCustomer(string $token, string $email): ?array
    {
        ...
        Log::warning('HS find failed', ['email'=>$email,'s1'=>$r->status(),'s2'=>$r2->status(),'b1'=>$r->body(),'b2'=>$r2->body()]);
        return null;
    }
    ...
    private function hsUpdateCore(string $token, int $id, array $b): void
    {
        ...
        if (!$r->successful()) {
            Log::warning('HS core update failed (non-fatal)', [
                'status'=>$r->status(),'body'=>$r->body(),'pay'=>$payload
            ]);
        }
    }
    ...
    {
        // Map our concepts → HS custom property slugs
        $slugMap = [
            'donor_id'             => 'donor-id',
        ...
        // Warn once if some slugs don’t exist (and silently skip them)
        $missing = [];
